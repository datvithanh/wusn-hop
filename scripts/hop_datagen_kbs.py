...
def hop_is_covered(sn, bs_, relays_, sensors_, radius, check_sensor_in_relay=True):
    if distance(sn, bs_) <= radius:
        return False
    
    if check_sensor_in_relay:
        for rn_ in relays_:
            if distance(sn, rn_) <= radius:
                return False
    ...
    for inp_ in args.dems:
        ...
        for r in radi:
            for i in range(args.count):
                ...
                fpath = os.path.join(args.output, 'small/layer', fname)
                res = WusnInput.from_file(fpath)
                # res.to_file(fpath)
                bs = res.BS
                sensors = res.sensors
                relays = res.relays

                fpath = os.path.join(args.output, 'small/hop', '%s%s_r%d_%d_0.json' % (args.prefix, dname, r, i+1))
                if os.path.exists(fpath):
                    print("existed")
                    continue
                res.to_file(fpath)
                ...
                for j in range(args.ns):
                    ...
                    while True:
                        sn = point(dem, (0, args.W), (0, args.H), z_off=-args.depth, cls=SensorNode, distribution=args.distribution)
                        if hop_is_covered(sn, bs, relays, sensors, 2 * r, check_sensor_in_relay=check):
                            break
                    sensors.append(sn)
                ...
                fpath = os.path.join(args.output, 'small/hop', '%s%s_r%d_%d_40.json' % (args.prefix, dname, r, i+1))
                res.to_file(fpath)
            # break
        # break
    logger.info('Done')
