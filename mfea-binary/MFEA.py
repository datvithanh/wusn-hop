...
from constructor.binary import Hop, Layer
...
def init_individual(relay_positions, num_selected_relays):
    rl = [np.random.randint(0, relay_positions) for _ in range(num_selected_relays)]
    individual = [0] * relay_positions
    for idx in rl:
        individual[idx] = 1
    return individual
...
def run_ga(hop_inp: WusnInput, layer_inp: WusnInput, logger=None):
    ...
    logger.info("Start!")
    num_selected_relays = 14

    hopConstructor = Hop(hop_inp, max_relay=num_selected_relays)
    layerContructor = Layer(layer_inp, max_relay=num_selected_relays)

    def factorial_rank(pop):
        factorial_cost = [(hopConstructor.get_loss(indi), layerContructor.get_loss(indi)) for indi in pop]

        print(len(pop))
        print('inf hop', sum([tmp[0] > 10 or tmp[1] > 10 for tmp in factorial_cost[:300]]))
        print('inf hop', sum([tmp[0] == float('inf') or tmp[1] == float('inf') for tmp in factorial_cost[300:]]))
        print(min([tmp[0] for tmp in factorial_cost]))
        print(min([tmp[1] for tmp in factorial_cost]))
    ...
    pop = [init_individual(layer_inp.num_of_relays, num_selected_relays) for _ in range(POP_SIZE)]
    ...
    best_obj2 = min(layerContructor.get_loss(best_individual1), layerContructor.get_loss(best_individual2))
